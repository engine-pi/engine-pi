...
public final class InfoBoxDrawer
{
    ...
    {
        DrawUtil.drawArrowLine(g, x, y, x + (int) gravity.getX() * 2,
            ...
            int actorsCount)
    {
        ...
        DrawUtil.drawTextBox(g, "FPS: "
                + (frameDuration == 0 ? "∞" : Math.round(1 / frameDuration)),
                10, colors.get("blue"));
        // Anzahl an Figuren
        DrawUtil.drawTextBox(g, "Actors: " + actorsCount, 50,
                colors.get("green"));
        // Schwerkraft
        ...
        if (!gravity.isNull())
        {
            DrawUtil.drawTextBox(g, String.format("G(x,y): %.2f,%.2f",
