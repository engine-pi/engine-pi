...
<!-- Go to file:///data/school/repos/inf/java/engine-pi/subprojects/demos/src/main/java/demos/docs/physics/impulse/DominosDemoBasic.java -->

Um die Grundlagen der Physics Engine zu testen, bauen wir eine einfache
...
{{ code('demos.docs.physics.impulse.DominosDemoBasic', from_class=True) }}
...
drückt sich in verschiedenen {{ javadoc('pi.physics.BodyType') }}s  aus:
...
`#!java Scene.setGracity(Vector)` eine Schwerkraft gesetzt, die auf den Ball und die
...
{{ code('demos.docs.physics.impulse.DominosDemoFinal') }}
